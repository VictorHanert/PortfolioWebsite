...
export const CONTINENT_TOTALS: Record<string, number> = CONTINENT_DATA.reduce((acc, continent) => {
    acc[continent.name] = continent.totalCountries;
    return acc;
}, {} as Record<string, number>);

export const WORLD_TOTAL_COUNTRIES = CONTINENT_DATA.reduce(
    (sum, continent) => sum + continent.totalCountries,
    0
);

export const RAW_VISITED_COUNTRIES: VisitedCountry[] = CONTINENT_DATA.flatMap(
    (continent) => continent.countries
);

export const getContinentForCountryId = (countryId: string): string | undefined => {
    return CONTINENT_DATA.find((continent) =>
        continent.countries.some((country) => country.id === countryId)
    )?.name;
};
